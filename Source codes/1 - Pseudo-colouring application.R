...
{  
  ...
  #select R-peaks as any peak larger than 0.8 mv
  ...
  #Lift the signal up to set the baseline to zero
  mv <- ECG$mv + 0.12
...
p  +  
  ...
  scale_y_continuous(lim = c(-0.05, +1)) +
  ...
  #facet_grid(~newcolourVector, scales = 'free_x', space = 'free') + 
  geom_line(size=0.7) + myColor_scale_fill +
